...
offense_team_down_run_pass_summary_3 |>
  ...
  tab_header(title = "Offensive Efficiency by Team - 3rd Down",
             subtitle = paste0(season1, " season. At least 0 seconds remaining in the half, win probability between ", min_win_prob, " and ", max_win_prob)
  ) |>
  ...
  tab_source_note(source_note = "Table by TripleRStats.")
...
compare_team_down_performance_to_league <- function(pbp_df, seasons = c(2025), 
...
create_team_offense_down_run_pass_gt <- function(comparison_df, min_wp = 0, max_wp = 1, min_half_seconds_remaining = 0, 
...
first_down_vars_highlight_list = c("pass_percent", "rush_epa_per_play", "pass_epa_per_play")
second_down_vars_highlight_list = c("pass_epa_per_play")
third_down_vars_highlight_list = c("success_rate")
